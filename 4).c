...
void Metropolis_c(char *config,double prob[],int xp[],int yp[],int xm[],int ym[],int r){
    ...
    FILE*f1;
    char filename[40];
    sprintf(filename, "M(t)%d_c.txt", r);
    f1=fopen(filename,"w");
    ...
    for(t=0;t<Nt;t++){
        ...
        //printf("%lf     %lf\n",(log(epsilon)/(log(1-sum_prob/V))),sum_prob/V);
...
void Metropolis(char *config,double prob[],int xp[],int yp[],int xm[],int ym[],int r){
    int ind,n,j,y,x,t;
    double w;
    double m;
...
int main (){
    ...
    config_c=(char*)calloc(V,sizeof(char));
    ...
    int xp_c[L]={0};int yp_c[L]={0};int xm_c[L]={0};int ym_c[L]={0};
    ...
    for (i=0;i<10;i++){
        configrandom(config,V);
        configrandom(config_c,V);
        moverse(xp,yp,xm,ym);
        moverse(xp_c,yp_c,xm_c,ym_c);
        Metropolis(config,prob,xp,yp,xm,ym,i);
        Metropolis_c(config_c,prob,xp_c,yp_c,xm_c,ym_c,i);
        fichero(config);
        fichero(config_c);
    }
    //moverse(xp,yp,xm,ym);
    //Metropolis_continuo(config,prob,xp,yp,xm,ym);
    //fichero(config);
    free (config); free (config_c);
